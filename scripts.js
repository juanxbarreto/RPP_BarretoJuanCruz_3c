...
document.addEventListener('DOMContentLoaded', function () {
    ...
    /***************************************************** MOD ******************************************************* */
    // Agrega un evento de escucha al botón "Modificar"
    const modificarButton = document.getElementById('modificarButton');
    modificarButton.addEventListener('click', function () {
        // Obtiene el valor del campo ID a modificar
        const idAModificar = parseInt(document.getElementById('id').value);

        // Busca el objeto en la lista de personajes por su ID
        const personajeAModificar = personajes.find((personaje) => personaje.id === idAModificar);

        // Verifica si se encontró un personaje con el ID especificado
        if (personajeAModificar) {
            ...
            // Actualiza los campos específicos según el tipo de personaje
            if (personajeAModificar instanceof Heroe) {
                const alterego = document.getElementById('alterego').value;
                const ciudad = document.getElementById('ciudad').value;
                const publicado = parseInt(document.getElementById('publicado').value);

                // Actualiza los valores del objeto
                personajeAModificar.nombre = nombre;
                personajeAModificar.apellido = apellido;
                personajeAModificar.edad = edad;
                personajeAModificar.alterEgo = alterego;
                personajeAModificar.ciudad = ciudad;
                personajeAModificar.publicado = publicado;
            } else if (personajeAModificar instanceof Villano) {
                const enemigo = document.getElementById('enemigo').value;
                const robos = parseInt(document.getElementById('robos').value);
                const asesinatos = parseInt(document.getElementById('asesinatos').value);
                ...
                personajeAModificar.apellido = apellido;
                personajeAModificar.edad = edad;
                personajeAModificar.enemigo = enemigo;
                personajeAModificar.robos = robos;
                personajeAModificar.asesinatos = asesinatos;
            }

            // Muestra un mensaje de éxito
            alert('Personaje modificado con éxito.');
            ...
            // Regresa al formulario de datos
            document.getElementById('formAbm').style.display = 'none';
            document.getElementById('formDatos').style.display = 'block';

            // Vuelve a mostrar la lista de objetos actualizada
            mostrarObjetos();
        } else {
            alert('No se encontró ningún personaje con el ID especificado.');
        }
    });

    /***************************************************** DELETE ******************************************************* */
    // Agrega un evento de escucha al botón "Eliminar"
    const eliminarButton = document.getElementById('eliminarButton');
    eliminarButton.addEventListener('click', function () {
        // Obtiene el valor del campo ID a eliminar
        const idAEliminar = parseInt(document.getElementById('id').value);

        // Busca el índice del objeto en la lista de personajes por su ID
        const indexAEliminar = personajes.findIndex((personaje) => personaje.id === idAEliminar);

        // Verifica si se encontró un personaje con el ID especificado
        if (indexAEliminar !== -1) {
            // Elimina el personaje de la lista por su índice
            personajes.splice(indexAEliminar, 1);
